...
**** TODO Serialized index value must include the primary key
**** TODO Index key must include unique suffix (e.g. ulid)
**** TODO Query must return an iterator that provides index key/value pairs
***** returns (key, doc_id, value) 3-tuple
***** strip the unique suffix from the index key
***** split the document key from the index value
